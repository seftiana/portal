...
function checkTableName($shortTName, $type=false)
{
	...
	if ("s_kelas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("s_krs_detil" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("mahasiswa_akun" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
//	default permissions
	if($table=="s_krs_detil")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="mahasiswa_akun")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("s_user")
