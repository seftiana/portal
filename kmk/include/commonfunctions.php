...
function checkTableName($shortTName, $type=false)
{
	...
	if ("mahasiswa_registrasi" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("mahasiswa" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pengguna" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("status_aktif_mahasiswa_ref" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
		$arr[]="mahasiswa_registrasi";
		$arr[]="mahasiswa";
		$arr[]="pengguna";
		$arr[]="status_aktif_mahasiswa_ref";
	return $arr;
}
...
//	default permissions
	if($table=="status_aktif_mahasiswa_ref")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
