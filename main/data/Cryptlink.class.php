 ...
 * 
 * This class can used for encryption and decryption of link of each web page
 * @package 
 * @author Suyud Widiono
...
$cryptObj = Encrypt::init(getNumericChar($sess_id));

function getNumericChar($str){
	$ret = '';
	for($i=0,$m=strlen($str);$i<$m;++$i)if(is_numeric($str{$i}))$ret .= $str{$i};
	return $ret;
}
...
function EncryptLink($value) {
	global $cryptObj;
	$enc = $cryptObj->enc($value);
	return $enc;
}

function DecryptLink($value) {
	global $cryptObj;
	$dec = $cryptObj->dec($value);
	return $dec;
}
...
class _Encrypt{
	...
	for ($i=0;$i<=$pj-1;$i++){
		...
		else if ($sample=="_") $enc .= "+";
		else if ($sample==" ") $enc .= "!";
		else if ($sample==".") $enc .= ".";
		else $enc .= $sample ;
	}
	$today      = md5(date("Y-m-d"));	// fixing at php5 "F j, Y, g:i a" is inconsistent
...
/* new Encrypt class
   fixing unstable old encryption proc
   on php5
   moezhank@gmail.com
   20131007
*/
...
private $salt;
private $debug = false;
public static function init($seq=0){
	if (!isset(self::$object)) {
		self::$object = new Encrypt($seq);
		self::$object->setKey($seq);
	}
	return self::$object;
}
