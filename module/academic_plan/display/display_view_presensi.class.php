<?php
   ...
   */
	...
	class DisplayViewPresensi extends DisplayBaseFull {
      var $mUserRole;
      var $mMahasiswaNiu;
      ...
      function DisplayViewPresensi(&$configObject, &$security, $userRole, $mhsniu, $prodiId, $errMsg, $viewBy, $siaServer,$getBlock) { #update ccp 9-juli-2019
         ...
         $this->SetTemplateFile('view_presensi.html');
      }
    ...
    if (false === $dataUser) {
        ...
        $this->SetAttribute('krs','visibility','hidden');
			...
			//$resPresensiKls = $this->mObjKrsService->getDataRekapPresensi($krsId,$this->mMahasiswaNiu);
			$tahunsemester = $this->mObjKrsService->GetProperty("SemesterProdiSemesterId");
			$resPresensiKls = $this->mObjKrsService->getDataRekapPresensi($tahunsemester,$this->mMahasiswaNiu);
			// echo"<pre>";print_r($resPresensiKls);

			if($resPresensiKls[0]['KELAS'] != ""){
				$this->AddVar('data_list','IS_EMPTY','NO');
				for ($i=0; $i < count($resPresensiKls); $i++) {
					$this->AddVar('data_list_item','NO',$i+1);
					$this->AddVar('data_list_item','KELAS',$resPresensiKls[$i]['KELAS']);
					$this->AddVar('data_list_item','MK',$resPresensiKls[$i]['MK']);
					$this->AddVar('data_list_item','SKS',$resPresensiKls[$i]['SKS']);
					$this->AddVar('data_list_item','KETERANGAN',$resPresensiKls[$i]['KETERANGAN']);
					$this->AddVar('data_list_item','JUMLAH',$resPresensiKls[$i]['JUMLAH']);
