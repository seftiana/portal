<?php
   ...
   if (false !== $security->CheckAccessRight($ThisPageAccessRight)) {
      $mhsNiu = NULL;
	  // echo"<pre>";print_r($_POST);echo"</pre>";exit;die;
      if (isset($_GET["niu"])){
      ...
      if ($_POST['act'] == 'krs') {
         if (isset($_POST['btnApproval'])) {
            ...
            if ($_POST['btnProses'] == 'Simpan Remedial') {
               // proses remedial krs yang diambil
			   // echo"<pre>";print_r($_POST); 
			   // echo"</pre>";die;exit;
               // if (!empty($_POST['kodeMkul'])) {
                  $result = $app->RemedialKrsItem($_POST['kodeMkul'],$_POST['krsId'][0]);
               // }
               $errMsg = $cfg->Enc($app->GetProperty("ProcessAcademicPlanError"));
               header("Location: " . $cfg->GetURL('academic_plan','academic_plan_remedial','view'). '&err='. $errMsg . 
                  $additionalUrl);
               exit;
            } 
         }
