...
class DisplayViewKHSOrtu extends DisplayBaseFull {
   var $mMahasiswaNiu;
   ...
   function DisplayViewKHSOrtu(&$configObject, &$security, $mhsniu, $prodiId, $sempId, $errMsg, $viewby, $siaServer, $getBlock) {
      ...
      $this->SetTemplateFile('view_khs_ortu.html');
   }
   ...
   function DisplayKhsForOneSemester(&$objAcademicReport, $dataUser) {
      $dataKhs = $objAcademicReport->GetAllKHSItemForSemesterOrtu();
      if  (!empty($dataKhs)) {
   ...
   function Display() { 
      ...
      if (false !== $dataUser) {
		...
		##add ccp 8-juli-2019
		}else if(($now >= $dataSemAktif[0]['INPUT_NILAI_MULAI'] && $now <= $dataSemAktif[0]['INPUT_NILAI_SELESAI']) OR ($now >= $dataSemAktif[0]['INPUT_NILAI2_MULAI'] && $now <= $dataSemAktif[0]['INPUT_NILAI2_SELESAI'])){ 			
