...
class DisplayInputPresenceClass extends DisplayBaseFull {
   
	function DisplayInputPresenceClass(&$configObject, &$security, $semesterId, $kelasId, $pertemuanId, $errMsg) {
		...
		$this->SetTemplateFile('input_presence_class.html');
	}
	...
	#add ccp 24-8-2019
    	...
    	function CekStatusTagihanPrasyaratMhs($nimMhs){
    		$status = false;
		$arrNimForPrasyarat = $nimMhs;
        	$dataTagihanPrasyaratMhs = $this->GetTagihanPrasyaratMahasiswa($arrNimForPrasyarat,$this->mSemesterId);
	...
	function GetTagihanPrasyaratMahasiswa($arrNim,$semId='') {
       		if($this->mFinansiServiceAddress == ''){
         		$this->ShowErrorBox("Pengambilan data tagihan tidak berhasil.");            
        	} else{
           		$restClient = new RestClient();
            		$restClient->SetPath($this->mFinansiServiceAddress.
			'?mod=lap_histori_pembayaran&sub=HistoriPembayaranMahasiswa&typ=rest&act=rest');
			$restClient->SetGet('&prasyarat=yes&smt='.$semId.'&nim='.json_encode($arrNim)); #akd
	...
	function Display() {
		...
		$this->AddVar("content", "TEMA", $dataPertemuan[0]['TEMA']); ##add ccp 5-12-2025
		$this->AddVar("content", "POKOK_BAHASAN", $dataPertemuan[0]['POKOK_BAHASAN']); ##add ccp 5-12-2025
		...
		if($dataPertemuan === false){
			...
			foreach($dataPeserta as $i => $peserta){
				...
				$peserta["NO"] = $no++;
				#add ccp 24-8-2019
				$statusTagihanPrasyaratMhs =  $this->CekStatusTagihanPrasyaratMhs($peserta['NIM']);
				if($statusTagihanPrasyaratMhs == true && $peserta['STATUS_SMT']=='SEMESTER_LAMA'){ 
					$peserta['BLOCK_ABSEN']= 'class="table-common-alert"';
					$peserta['BLOCK_TITLE']= 'ada hutang keuangan'; 
					$option.= "<option value=\"1\" selected >Alpa</option>";
				}else{
