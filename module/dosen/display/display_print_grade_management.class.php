<?php
/**
 * DisplayPrintGradeManagement
 * DisplayPrintGradeManagement class
 * 
 * @package dosen 
 * @author Maya Alipin
...
class DisplayPrintGradeManagement extends DisplayBasePrint {
   var $mErrorMessage;
   ...
   var $mAfterProc;
    /**
    * DisplayPrintGradeManagement::DisplayPrintGradeManagement
    * Constructor
    *
    ...
    */
   function DisplayPrintGradeManagement(&$configObject, &$security, $errMsg,$semester, $kelas, $serverAddress, $halaman, $perNif, $afterProc, $prodi_kode_kelas) {
   ...
   /**
    * DisplayPrintGradeManagement::GetStudentsGrade
               ...
               if ($pilihanNilai === false) {
                  ...
                  foreach ($dataNilai as $key=>$value) {
					...
					// if($nilaiDpna[0]['krsdtIsRemedial']==1){
					 ...
					 // }
				  	if($statusTagihanPrasyaratMhs == true || $nilaiDpna[0]['krsdtIsRemedial']!=1){
                     	$this->AddVar("nilai_dpna", "READ_ONLY", 'readonly');
						$this->AddVar("nilai_dpna", "REMED","hidden");
                     	# nilai diambil dari kelompok yang tertinggi                     	
                     	$nilaiArr = array();
