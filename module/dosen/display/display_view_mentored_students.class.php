 ...
 function ShowUnitOption(){
            ...
            if (false !== $mentoredStudents) {
               ...
               if (!empty($mentoredStudents)){
                  ...
                  foreach ($mentoredStudents as $key=>$value){
					...
					$nama_m = strtolower($mentoredStudents[$key]["nama"]);
					$prodi_m = strtolower($mentoredStudents[$key]["program_studi"]);
					$mentoredStudents[$key]["nama"] = ucwords($nama_m);
					$mentoredStudents[$key]["program_studi"] = ucwords($prodi_m);
					$ipk_m = $this->GetDataTranskrip($mentoredStudents[$key]["nim"]);
					$mentoredStudents[$key]["ipk"] = $ipk_m[0]['ipk'];
					$mentoredStudents[$key]["sks"] = $ipk_m[0]['totol_sks'];
                  }
				  // echo"<pre>";print_r($mentoredStudents);echo"</pre>";
   ...
   function ShowErrorBox() {
	...
	function GetDataTranskrip($nim){
		$students = $this->mDosenServiceObj->GetDataTranskrip($nim);
		if (false === $students) {
			return false;
		}else {
			return $students;
