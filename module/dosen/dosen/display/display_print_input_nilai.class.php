<?php
/**
 * DisplayPrintInputNilai
 * DisplayPrintInputNilai class
 * 
 * @package dosen 
 * @author Maya Alipin
...
class DisplayPrintInputNilai extends DisplayBasePrint {
   var $mErrorMessage;
   ...
   var $mAfterProc;
    /**
    * DisplayPrintInputNilai::DisplayPrintInputNilai
    * Constructor
    *
    ...
    */
   function DisplayPrintInputNilai(&$configObject, &$security, $errMsg,$semester, $kelas, $serverAddress, $halaman, $perNif, $afterProc, $prodi_kode_kelas) {
      ...
      $this->SetTemplateBasedir($configObject->GetValue('app_module') . 'dosen/templates');
	    ...
	    $this->SetTemplateFile('print_input_nilai.html');
		$this->mAfterProc=$afterProc;
	}
   
   
   /**
    * DisplayPrintInputNilai::GetStudentsGrade
      ...
      if ($init === false){
		...
		//  $resKodeNilai = $this->mDosenServiceObj->GetKodeNilai($this->mrUserSession->GetProperty("UserProdiId")); //hidden by ccp 16 mar 2017 prodinya harusnya sesuai kelasnya
         ...
         $dataNilai = $this->GetStudentsGrade();//print_r($dataNilai);exit;
			// echo'ahay';print_r($dataNilai);die;
         if (false !== $dataNilai) {
