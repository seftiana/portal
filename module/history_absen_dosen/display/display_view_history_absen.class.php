<?php
class DisplayViewHistoryAbsen extends DisplayBaseFull {
   ...
   /**
     * DisplayViewHistoryAbsen::DisplayViewHistoryAbsen
     * Constructor for DisplayViewHistoryAbsen
     *
     ...
     * @return void
     */
   function DisplayViewHistoryAbsen(&$configObject, &$security, $dsnNip, $prodiId, $viewFrom, $semesterId, $sia, $errMsg) {
      ...
      $this->SetTemplateFile('view_history_absen.html');
   }
   
   /**
     * DisplayViewHistoryAbsen::GetDataDosen
   ...
   /**
     * DisplayViewHistoryAbsen::GetDataSemesterAktif
   ...
   /**
     * DisplayViewHistoryAbsen::GetDaftarSemester
   ...
   /**
     * DisplayViewHistoryAbsen::ShowErrorBox
   ...
   /**
     * DisplayViewHistoryAbsen::ShowEmptyBox
   ...
   /**
     * DisplayViewHistoryAbsen::ShowSemesterList
   ...
   /**
     * DisplayViewHistoryAbsen::ShowUnitList
   ...
   function ShowUnitList($currServer) {
      ...
      if ($unit !== false) {
         ...
         $this->AddVar("option_form", "URL_VIEW", $this->mrConfig->GetURL('history_absen_dosen', 'absen', 'process'));
         if($this->mViewedFrom == 'nilai')
   ...
   /**
     * DisplayViewHistoryAbsen::Display
   ...
   function Display() {
      ...
      if (false !== $dataDosen && false !== $isUnitAvailable) {
         ...
         if(empty($this->mSemesterId)) {
			...
			$this->AddVar('content', 'JUDUL', 'History Absen Mengajar');
				  ...
				  if($isDosenMengajar) {
                     ...
                     if(false === $dataMatakuliahDiampu) {
                        ...
                        for($i = 0; $i < count($dataMatakuliahDiampu); $i++) {
                            ...
                            #$dataMatakuliahDiampu[$i]['url_presensi'] = $this->mrConfig->GetURL('kuisioner_dosen', 'presence_class', 'view').'&sem='.$this->mrConfig->Enc($semesterAktif).'&kls=' . $dataMatakuliahDiampu[$i]['id'];
                        }
