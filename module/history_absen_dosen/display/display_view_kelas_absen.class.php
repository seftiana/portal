<?php
/**
 * DisplayViewKelasAbsen
 * DisplayViewKelasAbsen class
 *
 * @package display
...
class DisplayViewKelasAbsen extends DisplayBaseFull {
   ...
   /**
     * DisplayViewKelasAbsen::DisplayViewKelasAbsen
     * Constructor for DisplayViewKelasAbsen
     *
     ...
     * @return void
     */
   function DisplayViewKelasAbsen(&$configObject, &$security, $dsnNip, $prodiId, $viewFrom, $semesterId, $sia, $errMsg) {
      ...
      $this->SetTemplateFile('view_kelas_absen.html');
   }
   
   /**
     * DisplayViewKelasAbsen::GetDataDosen
   ...
   /**
     * DisplayViewKelasAbsen::GetDataSemesterAktif
   ...
   /**
     * DisplayViewKelasAbsen::GetDaftarSemester
   ...
   /**
     * DisplayViewKelasAbsen::ShowErrorBox
   ...
   /**
     * DisplayViewKelasAbsen::ShowEmptyBox
   ...
   /**
     * DisplayViewKelasAbsen::ShowSemesterList
   ...
   /**
     * DisplayViewKelasAbsen::ShowUnitList
   ...
   function Display() {
      ...
      if (false !== $dataDosen && false !== $isUnitAvailable) {
         ...
         // cek apakah module ini ditampilkan dari module pengelolaan nilai
			...
			$this->AddVar('content', 'JUDUL', 'Matakuliah Diampu Semester '.$this->mSemesterId);
		...
		if(empty($this->mServiceServer)) {
		   ...
		   if(false != $semesterAktif) {
				if($isDosenMengajar) {
					...
					$dataMatakuliahDiampu = $objDosenService->GetAllKelasDosenForSemester();
					if(false === $dataMatakuliahDiampu) {
						...
						for($i = 0; $i < count($dataMatakuliahDiampu); $i++) {
						   $dataMatakuliahDiampu[$i]['id'] = $this->mrConfig->Enc($dataMatakuliahDiampu[$i]['id']);
							...
							$dataMatakuliahDiampu[$i]['url_presensi_cetak'] = $this->mrConfig->GetURL('history_absen_dosen', 'absensi_cetak', 'print').'&smt='.$this->mrConfig->Enc($dataMatakuliahDiampu[$i]['sem_kelas']).'&kelas=' . $dataMatakuliahDiampu[$i]['id'];
							$dataMatakuliahDiampu[$i]['CAPT_AKSI'] = "Lihat";
