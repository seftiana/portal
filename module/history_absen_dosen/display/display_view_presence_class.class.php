...
class DisplayViewPresenceClass extends DisplayBaseFull {
	...
	function Display() {
		...
		if($dataPertemuan === false){
			...
			if(empty($dataPertemuan)){
				...
				foreach($dataPertemuan as $pertemuan){
					...
					$pertemuan['LINK_PRESENSI_DETIL_INSERT_EDIT'] = $this->mrConfig->GetURL('history_absen_dosen', 'presence_class', 'edit').'&sem='.$this->mrConfig->Enc($this->mSemesterId).'&kls='.$this->mrConfig->Enc($this->mKelasId).'&ptm='.$this->mrConfig->Enc($pertemuan['PRESKLS_ID']);
		...
		$this->AddVar("content", "URL_BACK", $this->mrConfig->GetURL('history_absen_dosen','history_absen','view'));
		$url_print= $this->mrConfig->GetURL('history_absen_dosen', 'absensi_kelas', 'print') .
						'&kelas=' . $this->mrConfig->Enc($this->mKelasId) .
						'&smt=' . $this->mrConfig->Enc($this->mSemesterId) . 
                        '&sia=' . $this->mrConfig->Enc($serverUsed);
