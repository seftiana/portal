   ...
   function Display() {   	
      ...
      if ($init === false){
		...
		//  $resKodeNilai = $this->mDosenServiceObj->GetKodeNilai($this->mrUserSession->GetProperty("UserProdiId")); //hidden by ccp 16 mar 2017 prodinya harusnya sesuai kelasnya
         ...
         if (false !== $dataNilai) {
            $url_detail=$this->mrConfig->GetURL('history_nilai_dosen','grade_management','view')."&pBl=".$this->mrConfig->enc("detail")."&pGo=".$this->mrConfig->enc("detail")
...
//print_r($this->mDosenServiceObj);
      ...
      $this->AddVar("content", "URL_PROCESS", $this->mrConfig->GetURL("history_nilai_dosen", "dosen", "process"));
