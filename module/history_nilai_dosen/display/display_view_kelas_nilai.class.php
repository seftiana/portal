...
class DisplayViewKelasNilai extends DisplayBaseFull {
   ...
   /**
     * DisplayViewKelasNilai::ShowUnitList
					...
					if(false === $dataMatakuliahDiampu) {
						...
						for($i = 0; $i < count($dataMatakuliahDiampu); $i++) {
						   $dataMatakuliahDiampu[$i]['id'] = $this->mrConfig->Enc($dataMatakuliahDiampu[$i]['id']);
							...
							$dataMatakuliahDiampu[$i]['url_presensi'] = $this->mrConfig->GetURL('history_nilai_dosen', 'grade_management', 'view').'&smt='.$this->mrConfig->Enc($dataMatakuliahDiampu[$i]['sem_kelas']).'&kelas=' . $dataMatakuliahDiampu[$i]['id'].'&pkk='.$this->mrConfig->Enc($dataMatakuliahDiampu[$i]['prodi_kode_kelas']);
