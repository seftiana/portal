...
class DisplayViewInputPerbaikan extends DisplayBaseNoLinks {
   /**
   ...
   var $mAfterProc;
    /**
    * DisplayViewInputPerbaikan::DisplayViewInputPerbaikan
    * Constructor
    *
    ...
    */
   function DisplayViewInputPerbaikan(&$configObject, &$security, $errMsg,$semester, $kelas, $serverAddress, $afterProc, $prodi_kode_kelas,$subCpmk) {
	  ...
	  $this->SetTemplateFile('view_input_perbaikan.html');
	  $this->mAfterProc=$afterProc;
   }
   
   
   /**
    * DisplayViewInputPerbaikan::GetStudentsGrade
   ...
   function Display() {   	
      ...
      if ($init === false){
		...
		##end pagging
         ...
         if (($cekPeriode[0]["current_periode_input_nilai"] == "BUKANPERIODEINPUTNILAI") && $readOnly == "NO") {
			...
			$display_bPresentasi = (empty($dataKelas["detil"][0]["TTLBOBOT_PRESENTASI"]))?'hidden':'';
      ...
      $this->AddVar("content", "URL_PROCESS", $this->mrConfig->GetURL("history_nilai_obe", "dosen_perbaikan", "process"));
