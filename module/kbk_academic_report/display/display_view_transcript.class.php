...
class DisplayViewTranscript extends DisplayBaseFull {
   ...
   /**
     ...
     * @return void
     */
   function DisplayViewTranscript(&$configObject, &$security, $mhsniu, $prodiId, $errMsg, $viewBy, $siaServer) {
      ...
      if (false !== $dataUser) {
         ...
         if (false !== $dataTranskrip) {
            ...
            if ($ada === true) {
               ...
               //$this->AddVar('buttonCetak', 'URL_CETAK', $this->mrConfig->GetURL('academic_report','transkrip_mhs','print') .
               $this->AddVar('buttonCetak', 'URL_CETAK', $this->mrConfig->GetURL('kbk_academic_report','academic_transcript','print') .
                                       "&niu=" . $this->mrConfig->Enc($this->mMahasiswaNiu) . "&prodi=" . $this->mrConfig->Enc($this->mMahasiswaProdiId)."&opsi=".$this->mrConfig->Enc("pasca"));
               $this->AddVar('buttonCetak', 'URL_CETAK_TRANSKRIP', $this->mrConfig->GetURL('kbk_academic_report','transkrip_01','print') .
                                       "&niu=" . $this->mrConfig->Enc($this->mMahasiswaNiu) . "&prodi=" . $this->mrConfig->Enc($this->mMahasiswaProdiId));
               $this->AddVar('buttonCetak', 'URL_CETAK_MM', $this->mrConfig->GetURL('kbk_academic_report','transkrip_mhs','print') .
