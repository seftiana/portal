...
class DisplayViewCourseSupported extends DisplayBaseFull {
   ...
   function GetDataDosen() {
      ...
      if ($objUserService->IsError()) {
         ...
         $objUserService->SetProperty("UserRole", 9 );
   ...
   function ShowUnitList($currServer) {
      ...
      if ($unit !== false) {
         ...
         $this->AddVar("option_form", "URL_VIEW", $this->mrConfig->GetURL('kbk_dosen', 'dosen', 'process'));
         if($this->mViewedFrom == 'nilai')
                     ...
                     if(false === $dataMatakuliahDiampu) {
                        ...
                        for($i = 0; $i < count($dataMatakuliahDiampu); $i++) {
                           ...
                           if($this->mViewedFrom == 'nilai')
                              ...
                              $dataMatakuliahDiampu[$i]['urldetail'] = $this->mrConfig->GetURL('kbk_dosen', 'grade_management', 'view') .
                              ...
                              $module = 'kbk_dosen|course_supported|view';
                              $dataMatakuliahDiampu[$i]['urldetail'] = $this->mrConfig->GetURL('kbk_proposed_classes', 'proposed_classes_detail', 'view') .
