...
class DisplayViewProposedClassesSemester extends DisplayBaseFull {
   ...
   function ShowUnitOption () {
      ...
      if ($unit !== false) {
         if (!empty($unit)) {
            ...
            $this->AddVar('unit', 'URL_VIEW', $this->mrConfig->GetURL('kbk_proposed_classes','proposed_classes_semester','view'));
   ...
   **/
   ...
   {
      $listProdi = $this->mObjProposedClasses->GetAllProdiForAdministrasi();
      if (false === $listProdi) {
         return false;
      }
      else {
         ...
         if ($this->mDisplay !== false){
            ...
            if (false !== $dataMkul) {
               ...
               for($i=0;$i<count($dataMkul);$i++) {               
                  ...
                  $dataMkul[$i]["url_kelasdetil"] = $this->mrConfig->GetURL('kbk_proposed_classes','proposed_classes_detail','view')."&kelas=" . $this->mrConfig->Enc($dataMkul[$i]["kelas_id"]) . $urlAdditional;
