...
class DisplayViewProfile extends DisplayBaseFull 
{
    ...
    * @return void
    */
   function DisplayViewProfile(&$configObject, $securityObj, $userId, $userRole, $viewBy, $siaServer) 
   {
      ...
      if ($this->mUserRole == 1 || $this->mUserRole == 8) {
         $this->SetTemplateFile('view_profile_mhs.html');
      } else {
         $this->SetTemplateFile('view_profile_dosen.html');
      }
   }
   ...
   function Display() 
   {
      ...
      if ($this->mUserRole != 1 && $this->mUserRole != 8 && $this->mUserRole != 2 && $this->mUserRole != 9){
         $this->mErrorMessage .= "User role tidak sesuai.";
         $this->ShowErrorBox("NONE");
      }else{
         if ($this->mUserRole == 1 || $this->mUserRole == 8){
            $this->AddVar("content", "PROFILE_NIM", $this->mUserId);   
         } else if ($this->mUserRole == 2 || $this->mUserRole == 9) {
      ...
      if ($this->mViewBy == "dosen"){
         ...
         $this->AddVar("buttonBack", "URL_BACK", $this->mrConfig->GetURL('kbk_dosen','mentored_students','view').
