<?php
   ...
   require_once $cfg->GetValue('app_module') . 'kbk_user/display/process_user.class.php';
   require_once $cfg->GetValue('app_module') . 'kbk_user/business/user.class.php';
   
   $ThisPageAccessRight = "ADMINISTRASI | ADMINUNIT | MAHASISWA | DOSEN | MAHASISWA_KBK | DOSEN_KBK | ORTU";
      ...
      if ($doact!=="") {
         ...
         switch($doact){
            ...
            case "agreement":
               switch ($_POST["step"]) {
                  case "intro":
                     header("Location: ". $cfg->GetURL('kbk_user', 'agreement_disclaimer', 'view'). "&isIntro=". $cfg->Enc('1'));
                     break;
                  ...
                  case "disclaimer":
                     if (isset($_POST["btnTidakSetuju"])) {
                        ...
                        header("Location: ". $cfg->GetURL('kbk_user', 'agreement_chgpassword', 'view'). "&isDisc=". $cfg->Enc('1'));
                     }
                     break;
                  ...
                  case "chgpasswd":
                     ...
                     if(!empty($_POST["passwd1"]) && !empty($_POST["passwd2"])) {
                        if ($_POST["passwd1"] != $_POST["passwd2"]){
                           ...
                           header("Location: ".$cfg->GetURL('kbk_user', 'agreement_chgpassword', 'view')."&isDisc=".$encVar1."&err=".$encErrMsg);
                        } else {
                           ...
                           if(false === $result && $app->GetProperty("ProcessErrorType") == 'fatal') {
                              ...
                              if(false === $result && $app->GetProperty("ProcessErrorType") == 'fatal') {
                                 ...
                                 header("Location: ".$cfg->GetURL('kbk_user','agreement_greet','view')."&success=".$encVar1);
                              }                           
                           }
                        }
                     } else {
            ...
            case "password":
               if(!empty($_POST["passwd1"]) && !empty($_POST["passwd2"])) {
                  if ($_POST["passwd1"] != $_POST["passwd2"]){
                     ...
                     header("Location: ".$cfg->GetURL('kbk_user', 'user', 'chgpassword')."&errMsg=".$encErrMsg."&errType=".$errType);
                  }
                  else {
                     $result = $app->UpdateUserPassword($security->mUserIdentity->GetProperty("User"), $_POST["passwd2"], $_POST["oldPasswd"]);
                     if(false === $result && $app->GetProperty("ProcessErrorType") == 'fatal') {
            ...
            case "rstpwd":
               ...
               header("Location: " . $cfg->GetURL('kbk_user','user','rstpassword') . '&key='. $_POST['key'] . '&id='. $_POST['id'] . '&errMsg='. $errMsg . '&errType='. $errType . '&newPwd='. $newPwd);
               break;
            ...
            default :
               if(isset($_POST["btnTambah"])) {
                  ...
                  header("Location: ". $cfg->GetURL('kbk_user', 'user', 'view') . "&cari=" . $cfg->Enc($_POST["cari"]));
               }
               break;
         ...
         header("Location: ". $cfg->GetURL('kbk_user', 'agreement_intro', 'view'));
      }
   }else{
