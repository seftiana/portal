...
class DisplayKuisioner extends DisplayBaseNoLinks {
   ...
   var $semIdKuis; //add ccp 12-02-2019
   
   function DisplayKuisioner(&$configObject, &$security, $niu, $sia, $errMsg, $waktu, $semIdKuis) { //update ccp 12-02-2019
      ...
      $this->mSemIdKuis = $semIdKuis; //add ccp 12-02-2019
   ...
   function Display() {
	  ...
	  DisplayBaseNoLinks::Display();
		...
		if(empty($this->mServiceServer)) {
		 ...
		 $dataKuisioner = $objKuisionerService->GetKuisionerBySequence($seq,$this->mWaktu,$this->mSemIdKuis); //update ccp 12-02-2019
        ...
        if(!empty($dataKuisioner[0]['ID_PERTANYAAN'])) {
			...
			if($c>0){
				...
				$this->AddVar("mk_label", "MKNAMA", '\''.$dataMk[0]['NAMA_MK'].' - '.$dataMk[0]['NAMA_KELAS'].' :<br>'.$dataMk[0]['NAMA_DOSEN'].'\'');
				for($i=1;$i<$c;$i++){
					$this->AddVar("mk_list", "ID_MK", $this->mrConfig->Enc($dataMk[$i]['ID_MK']).'|'.$this->mrConfig->Enc($dataMk[$i]['NIP_DOSEN']));
					$this->AddVar("mk_list", "MKNAMA", $dataMk[$i]['NAMA_MK'].' - '.$dataMk[$i]['NAMA_KELAS'].' :<br>'.$dataMk[$i]['NAMA_DOSEN']);
			...
			foreach($sortedDataKuisioner as $grup=>$dataKuisionerJawaban){
				...
				foreach($dataKuisionerJawaban as $jawaban=>$data){
					...
					if($countKuisinonerTtl == count($_SESSION['jwbn']['lain'])){
						...
						$this->AddVar("pertanyaan_list_komentar", "PERTANYAAN", 'Isi Komentar');
			...
			if(!empty($_SESSION['jwbn'])){
				...
				$res = $objKuisionerService->InsertJawaban($arrRes,$this->mWaktu,$this->mSemIdKuis); //update ccp 12-02-2018
				unset($_SESSION['jwbn']);
				$this->mTemplate->setAttribute('endNote','visibility','visible');
			} else{
				// header("Location: ". $this->mrConfig->GetURL('home', 'home', 'view')); //hidden by ccp 11-04-2019
				#add ccp 11-04-2019
				if($this->mrUserSession->GetProperty('Role') != 1){
					header("Location: ". $this->mrConfig->GetURL('home', 'home', 'view'));
				}else{
					header("Location: ". $this->mrConfig->GetURL('kuisioner', 'informasi', 'view'));
				}
				#end
			}
		}
