<?php
   ...
   require_once $cfg->GetValue("app_module") . "kuisioner/display/display_survei.class.php" ; 
   ...
   if (false !== $security->CheckAccessRight($ThisPageAccessRight)){
	  ...
	  $niu = NULL;
      if (isset($_GET["niu"])){
         $niu = $cfg->Dec($_GET["niu"]);
      }
	  $prodi = NULL;
      if (isset($_GET["prodi"])){
         $prodi = $cfg->Dec($_GET["prodi"]);
      }
		
		
      $ThisPage = new DisplaySurvei($cfg, $security, $niu, $servServer, $errMsg, $prodi); //update ccp 12-02-2019 23-10-2018
      $ThisPage->Display();
   }else{
