...
class DisplayViewProposedClassesSemester extends DisplayBaseFull {
   ...
   **/
   ...
   function Display() {
      ...
      if (false === $init){
         ...
         if ($this->mDisplay !== false){
			...
			#add ccp 9-juli-2019
            $approvalKrs = $this->GetApprovalKrs($mhsNiu); 
			if($approvalKrs[0]['krsApprovalKe']=='0' or $approvalKrs[0]['krsApprovalKe']==''){
				$this->addVar('matakuliah_item','APPKRS','none');
			}else{
				// $this->addVar('matakuliah_item','APPKRS','');
				$this->addVar('matakuliah_item','APPKRS','none');
			}
			#end
   ...
   function ShowErrorBox() {
	...
	#add ccp 9-juli-2019
	function GetApprovalKrs($mhsNiu)
	{
		$krsItem = $this->mObjProposedClasses->GetApprovalKrsProposedClassesSemester($mhsNiu);
		if (false === $krsItem) {      	 
			return false;
		}else {      	 
			return $krsItem;
