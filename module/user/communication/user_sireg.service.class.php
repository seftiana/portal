...
class UserSiregService extends SiregSettingClientService{
	...
	function DoUploadFotoMahasiswa($niu, $varPost){
		$post['func'] =  'DoUploadFotoMahasiswa';
		$post['arg'] = json_encode($varPost);
		$post['niu'] = $niu;
		$ret = $this->Post($post);
		// $this->Debug();die;
		return $ret;
	...
	#add ccp 1-9-2020
	function DoUpdateBiodataMahasiswaHP($niu){
		$post['func'] =  'DoUpdateBiodataMahasiswaHP';
		$post['arg'] = json_encode($_POST);
		$post['niu'] = $niu;
		$ret = $this->Post($post);
		// $this->Debug();die;
