...
function checkTableName($shortTName, $type=false)
{
	...
	if ("t_user" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_unit" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("program_studi" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_hak_akses_ref" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
//	default permissions
	if($table=="program_studi")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="t_hak_akses_ref")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
